...
void Cartridge::loadArrayByteCode(uint8_t* code, uint32_t size) {
    uint8_t* oldRom = this->rom;
    this->romSize = size;
    this->rom = new uint8_t[size];
...
void Cartridge::extractHeaderData() {
    char *nameuint8_t = new char[16];
    
	for ( int32_t i = 0; i < 15; i++ ) {
        nameuint8_t[i] = rom[i+0x0134];
    }
    title = nameuint8_t;
...
int32_t Cartridge::getROMBanks() const {
    return this->romBanks;
}

int32_t Cartridge::getRamBanks() const {
