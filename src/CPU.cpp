...
uint8_t CPU::getCurrentOpcode() {
    uint8_t opcode = memory->read(PC.value);
...
void CPU::updateTimers(int32_t cycles) {
	divRegister += cycles ;
    
    uint8_t timerAtts = memory->readDirectly(0xFF07);
	if ( isBitSet(timerAtts, 2) ) {
		...
		while ( timaCounter >= currentClockSpeed ) {
			...
			if ( memory->readDirectly(0xFF05) == 0xFF ) {
				...
				uint8_t value = memory->readDirectly(0xFF05) + 1;
	...
	if (divRegister >= 256) {
		divRegister -= 256;
        uint8_t value = memory->readDirectly(0xFF04) + 1;
		memory->writeDirectly(0xFF04, value);
	}
}

void CPU::setCurrentClockSpeed(const int32_t speed) {
    this->currentClockSpeed = speed;
}
int32_t CPU::getCurrrentClockSpeed() const {
...
void CPU::updateInterrupts(short &cycles) {
	uint8_t requestFlag = memory->read(IF_REGISTER);
	if ( requestFlag > 0 ) {
			if (ime) {
				for (int32_t bit = 0; bit < 8; bit++) {
					if (isBitSet(requestFlag, bit)) {
						uint8_t enabledReg = memory->read(IE_REGISTER);
...
void CPU::serviceInterrupt(Interrupts interrupt) {
	...
	ime = false;
    uint8_t ifRegister = memory->readDirectly(IF_REGISTER);
...
void CPU::checkZeroFlag(const uint8_t data) {
    if ( data  == 0x00 ) {
        raiseFlag(ZERO_FLAG);
    } else {
        clearFlag(ZERO_FLAG);
...
void CPU::checkZeroFlag(const uint16_t data) {
    if ( data  == 0x0000 ) {
        raiseFlag(ZERO_FLAG);
    } else {
        clearFlag(ZERO_FLAG);
...
void CPU::checkHalfCarryFlag(const uint8_t data) {
    if ( (data & 0x0F) == 0x00 ) {
...
void CPU::checkHalfCarryFlag(const uint16_t data, const uint16_t number) {
...
void CPU::checkCarryFlag(const uint8_t data) {
    if ( (data & 0x100) == 0x100 ) {
...
void CPU::checkCarryFlag(const uint16_t data) {
    if ( data & 0x10000 ) {
        raiseFlag(CARRY_FLAG);
    } else {
        clearFlag(CARRY_FLAG);
...
void CPU::requestInterrupt(Interrupts interrupt) {
    uint8_t ifRegister = memory->read(IF_REGISTER);
