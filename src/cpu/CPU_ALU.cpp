...
void CPU::increment8BitRegister(uint8_t *reg) {
	*reg = *reg + 1;
...
short CPU::opcode0x34() {
    uint8_t result = memory->read(HL.value);
...
void CPU::decrement8BitRegister(uint8_t *reg) {
	*reg = *reg - 1;
...
short CPU::opcode0x35() {
    uint8_t data = memory->read(HL.value);
...
void CPU::xor8BitRegister(uint8_t *accumulator, const uint8_t data) {
...
short CPU::opcode0xEE() {
    uint8_t data = memory->read(PC.value);
    incrementProgramCounter();
    xor8BitRegister(&AF.hi, data);
    return 8;
}// XOR A, n 8

void CPU::or8BitRegister(uint8_t *accumulator, uint8_t data) {
	*accumulator = *accumulator | data;
...
void CPU::and8BitRegister(uint8_t *accumulator, uint8_t data) {
	*accumulator = *accumulator & data;
...
void CPU::add8BitRegister(uint8_t *accumulator, uint8_t adding) {
...
void CPU::adc8BitRegister(uint8_t *accumulator, uint8_t adding) {
	...
	uint16_t result = *accumulator + carryValue + adding;
...
void CPU::sub8BitRegister(uint8_t *accumulator, uint8_t substract) {
...
void CPU::sbc8BitRegister(uint8_t *accumulator, uint8_t substract) {
	int32_t carry = checkFlag(Flag::CARRY_FLAG) ? 1 : 0;
	int32_t result = *accumulator - substract - carry;
...
void CPU::compare8BitRegister(uint8_t accumulator, uint8_t data) {
	clearFlags();
...
short CPU::opcode0xE8() {
	...
	SP.value += static_cast<uint16_t>(data);
