...
short CPU::opcode0x17() {	
	uint8_t significantBit = AF.hi & 0x80;
	AF.hi = AF.hi << 1;
...
short CPU::extendedOpcode0x06() {
	uint8_t reg = memory->read(HL.value) ;
	rlc8bitRegister(&reg);
...
void CPU::rl8bitRegister(uint8_t *reg) {
	uint8_t significantBit = *reg & 0x80;
	*reg = *reg << 1;
